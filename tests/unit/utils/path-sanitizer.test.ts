...
describe('path-sanitizer utilities', () => {
  test('sanitizePath: valid simple path', () => {
  ...
  test('sanitizePath: detects traversal attempts', () => {
    const res = sanitizePath('../etc/passwd');
    expect(res.isValid).toBe(false);
    expect(res.errors).toContain('Path traversal attempt detected');
  });

  test('sanitizePathOrThrow throws on invalid path', () => {
    expect(() => sanitizePathOrThrow('../../../secret')).toThrow();
  });

  test('isPathWithinBase returns true/false correctly', () => {
  ...
  test('validateDownloadPath resolves existing file and rejects missing', async () => {
  ...
  test('validateMultiplePaths and areAllPathsValid behave correctly', () => {
