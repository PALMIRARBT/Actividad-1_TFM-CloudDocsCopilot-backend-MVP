...
jest.mock('../../../src/models/membership.model', () => ({ __esModule: true, default: { findOne: mockMembershipFindOne, countDocuments: mockMembershipCount, create: mockMembershipCreate }, MembershipRole: { MEMBER: 'member', ADMIN: 'admin', OWNER: 'owner', VIEWER: 'viewer' }, MembershipStatus: { PENDING: 'pending', ACTIVE: 'active' } }));
...
describe('membership.service invitations', () => {
  it('createInvitation validates userId format', async () => {
    ...
    await expect(createInvitation({ userId: 'bad', organizationId: 'o1', invitedBy: 'i1' } as any)).rejects.toThrow();
  });
  ...
  it('createInvitation handles organization not found', async () => {
    ...
    const { createInvitation } = require('../../../src/services/membership.service');
    await expect(createInvitation({ userId: '507f1f77bcf86cd799439011', organizationId: 'o1', invitedBy: '507f1f77bcf86cd799439012' })).rejects.toThrow('Organization not found');
  });

  it('createInvitation handles user/inviter missing', async () => {
    const org = { _id: 'o1', active: true, settings: { maxUsers: -1 }, plan: 'free', name: 'Org', slug: 'org' };
    mockOrgFindById.mockReturnValue({ populate: jest.fn().mockResolvedValue(org) });
    mockUserFindById.mockResolvedValueOnce(null);
    const { createInvitation } = require('../../../src/services/membership.service');
    await expect(createInvitation({ userId: '507f1f77bcf86cd799439011', organizationId: 'o1', invitedBy: '507f1f77bcf86cd799439012' })).rejects.toThrow('User not found');
  });
  ...
  it('createInvitation returns membership and sends email on success', async () => {
    ...
    const membership = { _id: 'm1', populate: jest.fn().mockResolvedValue({ _id: 'm1', organization: { name: 'Org' } }) };
    mockMembershipCreate.mockResolvedValue(membership);

    const { createInvitation } = require('../../../src/services/membership.service');
    const res = await createInvitation({ userId: '507f1f77bcf86cd799439011', organizationId: 'o1', invitedBy: '507f1f77bcf86cd799439012' });
