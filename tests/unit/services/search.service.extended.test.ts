...
const mockClient = {
  ...
  delete: jest.fn().mockImplementation(() => { throw { meta: { statusCode: 404 } }; }),
  search: jest.fn().mockResolvedValue({
    hits: {
      hits: [
        { _id: '1', _score: 1.2, _source: { filename: 'a', originalname: 'a', mimeType: 'text/plain', uploadedBy: 'u' } },
        { _id: '2', _score: 0.5, _source: { filename: 'b', originalname: 'b', mimeType: 'text/plain', uploadedBy: 'u' } }
      ],
      total: { value: 2 }
    },
    took: 5
...
describe('SearchService - Extended Coverage', () => {
  it('indexes and removes without throwing on 404', async () => {
    const doc: any = { _id: { toString: () => 'doc1' }, filename: 'f', originalname: 'f', mimeType: 'text/plain', size: 10, uploadedBy: { toString: () => 'u' }, organization: null, folder: null, uploadedAt: new Date() };
  ...
  it('searchDocuments returns mapped results and respects filters', async () => {
    ...
    const res = await svc.searchDocuments({ query: 'a', userId: 'u', organizationId: 'org1', mimeType: 'text/plain', fromDate: new Date('2020-01-01'), toDate: new Date('2022-01-01'), limit: 10, offset: 0 });
  ...
  it('getAutocompleteSuggestions dedupes and returns strings', async () => {
    ...
    (mockClient.search as jest.Mock).mockResolvedValueOnce({
      hits: { hits: [ { _source: { originalname: 'dup' } }, { _source: { filename: 'dup' } }, { _source: { filename: '' } } ], total: { value: 3 } }, took: 1
    });
